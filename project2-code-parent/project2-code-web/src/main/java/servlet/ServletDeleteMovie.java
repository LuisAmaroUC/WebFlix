...
public class ServletDeleteMovie extends HttpServlet {
    @EJB
    movieS print;
    @EJB
    userServices check;
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
       ...
       try{
        ...
        if((boolean)request.getSession().getAttribute("loggedIn")){
            if(checkManager){
                List<movieDTO> movies = print.getAll();
                ...
                if(movies.size() > 0) {
                    ...
                    out.println("<form id=\"1\" action=\"apagarMovies\" method=\"POST\">" +
           ...
           out.println("alert('O id do filme não existe');");
